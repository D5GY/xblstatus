...
client.on(Events.ClientReady, async () => {
  ...
  await xbls.utils
    .loadInteractions(xbls.commands, xbls.buttons)
    .catch((error) => console.error(error))
    .then(() => console.log('Loaded Interactions'));
  connectWS(client);
});

const INTERACTION_USAGE_WEBHOOK = new WebhookClient({
  url: config.WEBHOOKS.INTERACTION_USAGE,
});
client.on(Events.InteractionCreate, async (interaction) => {
  INTERACTION_USAGE_WEBHOOK.send({
    embeds: [
      xbls.utils.defaultEmbed(client, xbls.Colors.BLUE).addFields(
        {
          name: 'Type',
          value: interaction.type == 2 ? 'Command' : 'Button',
          inline: true,
        },
        {
          name: 'Name',
  ...
  if (interaction.type === InteractionType.ApplicationCommand) {
    const interactionExecuted = (await xbls.commands.get(
      interaction.commandName
    )) as any;
      ...
      await interaction.reply({
        ...
        embeds: [
          xbls.utils
            ...
            .setDescription(
              `An unknown error occurred, join our ${hyperlink(
                'support discord',
                config.MAIN_GUILD_INVITE_URL
              )} and report it! [!interactionExecuted]`
            ),
        ],
      });
    }
          ...
          xbls.utils
            ...
            .setDescription(
              `An unknown error occurred, join our ${hyperlink(
                'support discord',
                config.MAIN_GUILD_INVITE_URL
              )} and report it! [interactionExecuted.execute()]`
            ),
        ],
      });
    }
          ...
          xbls.utils
            ...
            .setDescription(
              `An unknown error occurred, join our ${hyperlink(
                'support discord',
                config.MAIN_GUILD_INVITE_URL
              )} and report it! [buttonExecuted.execute()]`
            ),
        ],
      });
    }
...
const GUILD_JOIN_WEBHOOK = new WebhookClient({
  url: config.WEBHOOKS.GUILD_JOIN,
});
    ...
    embeds: [
      {
        ...
        fields: [
          ...
          {
            name: 'Guild Owner / ID',
            value: `${guildOwner.user}\n${guildOwner.user.tag}\n${guild.ownerId}`,
            inline: true,
          },
        ],
        thumbnail: { url: guild.iconURL() ?? client.user!.avatarURL()! },
        footer: {
          text: `Guild Count: ${client.guilds.cache.size} | Guild Mem Count: ${guild.memberCount}`,
        },
      },
    ],
  });
});
const GUILD_LEAVE_WEBHOOK = new WebhookClient({
  url: config.WEBHOOKS.GUILD_LEAVE,
});
    ...
    .send({
      ...
      embeds: [
        xbls.utils
          ...
          .setFields({ name: 'Warn Message', value: message }),
      ],
    })
    .catch((error) => {
      throw error;
